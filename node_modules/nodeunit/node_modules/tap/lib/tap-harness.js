...
// Every "tests" is also a harness.  If they do not have a harness,
...
// - "skip" in the tests config obj should skip it.
...
Harness.prototype.process = function () {
  ...
  // to the next tests if the current one is actually over.
...
Harness.prototype.end = function () {
  ...
  if (this._plan !== null) {
    ...
    if (total !== this._plan) {
      this.results.add(assert.equal(total, this._plan, "tests count != plan"))
    }
    this._plan = total
  }
...
Harness.prototype.childEnd = function (child) {
  ...
  this.results.add(child.conf.name || "(unnamed tests)")
...
Harness.prototype.test = function test (name, conf, cb) {
  ...
  //console.error("making tests", [name, conf, cb])
