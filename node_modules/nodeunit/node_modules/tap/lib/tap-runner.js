...
function Runner (options, cb) {
  ...
  // Temporary tests files bunkerified we'll remove later
  this.f2delete = []
...
Runner.prototype.run = function() {
  ...
  function finish (er) {
    ...
    // Cleanup temporary tests files with coverage:
...
// glob the filenames so that tests/*.js works on windows
...
// Prepend to every tests file to run. Note tap.tests at the very top due it
// "plays" with include paths.
Runner.prototype.coverHeader = function() {
  // semi here since we're injecting it before the first line,
  // and don't want to mess up line numbers in the tests files.
  return "var ___TAP_COVERAGE = require("
       + JSON.stringify(require.resolve("runforcover"))
       + ").cover(/.*/g);"
}

// Append at the end of every tests file to run. Actually, the stuff which gets
...
Runner.prototype.coverFooter = function() {
  ...
  , "___TAP.tests(" + JSON.stringify("___coverage") + ", function(t) {"
