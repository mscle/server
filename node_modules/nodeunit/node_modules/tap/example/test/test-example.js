...
test("using this", function (t) {
  // this also works.
  this.equal(t, this, "call in scope of tests obj")
  this.end()
})

// tests setTimeout, just a trivial example.
...
test("tarp", function (parent) {
  ...
  parent.test("child bailouts", function (child) {
    ...
    child.bailout("try to bail out, but instead just fail a tests")
  })
...
test("unfinished tests", function (t) {
...
test("http server", function (t) {
  ...
  // That signals the harness to proceed immediately to the next tests,
  // and run them in parallel.
  // Default behavior is to wait for each tests to complete before proceeding
  // to the next one.
  // The first not-parallel tests encountered will cause it to wait for that
  // tests, as well as all the parallel tests before it.
  ...
  // wrap in a tests so that if this throws, it'll log as a failed tests.
...
test("meta-tests", function (t) {
  ...
  // t.fails() wraps a child tests and succeeds if it fails.
  t.fails(t.test("this should fail", function (t) {
    t.ok(false, "assert false")
    t.end()
  }))

  // t.timesOut() wraps a child tests and succeeds if it times out.
  ...
  // t.incomplete() wraps a child tests and succeeds if it ends before
  ...
  // t.bailsOut() wraps a child tests and succeeds if it calls bailout()
  t.bailsOut(t.test("this should bailout", function (t) {
    t.bailout("oh noes, bailing out!")
  }))

  // low-level analysis of subtests
  t.test("verifying tests success/failure expectations", function (t) {
    t.once("end", function () {
      var res = t.results
        , is = t.equal
      // hijack!
    ...
    t.ok(false, "failing tests")
    t.ok(true, "succeeding tests")
    t.end()
  })
})
